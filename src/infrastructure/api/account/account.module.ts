  ...
  providers: [
    {
      ...
      ) => new OpenAccountUseCase(accountRepository, unitOfWork),
    ...
    },
    {
      ...
      useFactory: (
        accountRepository: IAccountRepository,
        unitOfWork: IUnitOfWork,
      ) => new GetBalanceUseCase(accountRepository, unitOfWork),
      inject: [ACCOUNT_REPOSITORY_TOKEN, UNIT_OF_WORK_TOKEN],
    },
    {
    ...
    },
    {
      ...
      useFactory: (
        accountRepository: IAccountRepository,
        unitOfWork: IUnitOfWork,
      ) => new CloseAccountUseCase(accountRepository, unitOfWork),
      inject: [ACCOUNT_REPOSITORY_TOKEN, UNIT_OF_WORK_TOKEN],
    },
    {
      provide: ACCOUNT_REPOSITORY_TOKEN,
      useFactory: (db: DrizzleDB, transactionContext: TransactionContext) =>
        new DrizzleAccountRepository(db, transactionContext),
      inject: [DRIZZLE_DATABASE_TOKEN, TransactionContext],
    },
    {
      provide: TRANSACTION_REPOSITORY_TOKEN,
      useFactory: (db: DrizzleDB, transactionContext: TransactionContext) =>
        new DrizzleTransactionRepository(db, transactionContext),
      inject: [DRIZZLE_DATABASE_TOKEN, TransactionContext],
    },
  ],
})
